...
.PHONY: test
test:
	PYTHONPATH=$(PYTHONPATH) poetry run pytest -c pyproject.toml --cov-report=html --cov=conversation_clustering tests/
...
check-codestyle:
	poetry run darglint --verbosity 2 conversation_clustering tests
...
check-safety:
	poetry check
	poetry run bandit -ll --recursive conversation_clustering tests
